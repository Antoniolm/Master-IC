...
public class FeedForwardNetwork extends Layer{
    ...
    public double [] train(float [][]data,int label,double eta){
        double [] result=forward(convertImage(data)); //process image 
   ...
   public double [] convertImage(float [][]data ){
    ...
    public double [] crossEntropy(int valueC,double [] outPt){
        double [] error=new double[outPt.length];
        ...
        for(int i=0;i<outPt.length;i++)
            error[i]=-target[i] * log(outPt[i]) - (1.0-target[i])*log(1.0-outPt[i]);
